(function(ns) {
    ...
    ns.setupConfig = function({buttonId, config}) {
        ...
        ns.configs[buttonId].modelId = config.modelId;
    ...
    ns.createDOMElements = function({buttonId, config, loaderDiv}) {
        ...
        if (config.buttonType === ns.ButtonType.Default) {
            $buttonContainer.classList.add('QBTN-TYPE-Default');
            ...
            for (let i = 0; i < 4; i++) {
                ...
                $buttonComponent.id = ns.configs[buttonId].prefix + 'button-component-' + i;
                $buttonComponent.classList.add(ns.configs[buttonId].prefix + 'button-component');
    ...
    ns.initializeButton = function({$buttonLoader}) {
        ...
        ns.setupConfig({buttonId: buttonId, config: config});
        const cssPathCommon = `https://${ns.configs[buttonId].host}/css/type-common.css`;
        ns.loadCSS({cssPath: cssPathCommon, callback: ()=> {
            const cssPath = `https://${ns.configs[buttonId].host}/css/type-${config.buttonType}.css`;
