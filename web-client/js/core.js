    ...
    ns.Core = class {
        ...
        async connect(onConnected) {
            ...
            try {
                ...
                this.decoder = new window["opus-decoder"].OpusDecoder({ sampleRate: ns.configs[this.buttonId].decodeSampleRate, channels: 1 });
        ...
        playBufferedAudio() {
            ...
            if (this.audioBufferQueue.length === 0) {
                ...
                //DEBUG: 
                console.log("audioBufferQueue is empty, checking playbackQueue...");
                if (this.playbackQueue.length > 0) {
                    //DEBUG: 
                    console.log("Moving next item from playbackQueue to audioBufferQueue");
                    let nextAudioBuffers = this.playbackQueue.shift();
                    if (nextAudioBuffers && nextAudioBuffers.every(buffer => buffer.length > 0)) {
                        this.audioBufferQueue = nextAudioBuffers;
                        this.playBufferedAudio();
                    } else {
                        console.error("Invalid audio data in playbackQueue.");
                    }
                } else {
                    //DEBUG: 
                    console.log("playbackQueue is also empty, no more audio to play");
                }
                return; 
            }
            ...
            //DEBUG: 
            console.log("Starting audio playback with combined buffer");
